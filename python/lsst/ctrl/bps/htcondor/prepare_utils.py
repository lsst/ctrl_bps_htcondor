    ...
    if gwjob.number_of_retries is not None and gwjob.number_of_retries >= 0:
        ...
        if gwjob.number_of_retries > 0:
            periodic_release = _create_periodic_release_expr(
                gwjob.request_memory,
                gwjob.memory_multiplier,
                memory_max,
                user_release_expr,
            )
            if periodic_release:
...
def _translate_command_line(
    ...
    """Make environment, executable and argument settings for job.
    ...
    Returns
    -------
    jobcmds : `dict` [`str` `Any`]
        Commands to add to HTC submit description.
    """
    jobcmds = {}
    ...
    if gwjob.environment:
        ...
        for name, value in gwjob.environment.items():
            ...
            if use_htc_env:
                ...
                job_exports += f"export {name}='{value}';"
...
def _replace_cmd_vars(arguments, gwjob):
    ...
    try:
        ...
        _LOG.error(
            "Could not replace command variables for job %s: replacement for %s not provided",
            gwjob.name,
            str(exc),
        )
...
def _gather_label_values(config: BpsConfig, label: str) -> dict[str, Any]:
    ...
    """
    _LOG.debug("_gather_label_values: label = %s", label)
    values: dict[str, Any] = {"attrs": {}, "profile": {}}

    search_opts = config.get_search_opts(label)
    ...
    if found and not value:
        search_opts["skipNames"] = {"gwjobCommand", "gwjobExports"}
    ...
    found, value = config.search("bpsUseHTCEnvironment", opt=search_opts)
    values["bpsUseHTCEnvironment"] = value if found else values["bpsMakeCommand"]

    found, value = config.search("nodeset", opt=search_opts)
    if found:
        values["nodeset"] = value

    found, profile_sect = config.search("profile", opt=search_opts)
    if found and "condor" in profile_sect:
        for subkey, val in profile_sect["condor"].items():
            if subkey.startswith("+"):
                values["attrs"][subkey[1:]] = val
            else:
                values["profile"][subkey] = val

    # Copy all of the site values.
    if "curr_site" in search_opts["curvals"]:
        site_obj = config[f".site.{search_opts['curvals']['curr_site']}"]
        if site_obj:
...
def _generic_workflow_to_htcondor_dag(
    ...
    for job_name in generic_workflow:
        ...
        if gwjob.node_type in [
            GenericWorkflowNodeType.PAYLOAD,
            GenericWorkflowNodeType.LAZY_GROUP,
        ]:
            gwjob = cast(GenericWorkflowJob, gwjob)
            if gwjob.label not in cached_values:
                cached_values[gwjob.label] = _gather_label_values(config, gwjob.label)
        ...
        if gwjob.node_type == GenericWorkflowNodeType.GROUP:
            ...
            if not gwjob.blocking:
                ...
                check_job = _create_check_job(
                    f"wms_{gwjob.name}", gwjob.label, cached_values.get(gwjob.label, {})
                )
    ...
    if final and isinstance(final, GenericWorkflowJob):
        if final.label not in cached_values:
            cached_values[final.label] = _gather_label_values(config, final.label)
...
def _add_lazy_placeholder(
    ...
    job.add_job_attrs({"bps_job_name": "placeholder", "bps_job_label": "placeholder"})
    job.add_job_attrs(generic_workflow.run_attrs)

    # Placeholder dag name needs to be the run name because that's
    # currently what the bps code will name the workflow.
    ...
    _LOG.debug("_add_lazy_placeholder: edges = %s", dag.edges)


def _update_job_summary(subworkflow_name: str, subworkflow_summary: str, submit_path: str) -> None:
    """Add summary for jobs in given subworkflow to parent workflow's job
    summary.
    ...
    subworkflow_name : `str`
        Name of subworkflow.
    subworkflow_summary : `str`
        Job summary for the subworkflow.
    submit_path : `str`
        Directory in which to find the DAG info file.
    """
    _LOG.debug("submit_path = %s", submit_path)
    filename, dag_info = read_dag_info(submit_path)
    ...
    generator_name = None
    lazy_mapping = dag_values.get("bps_lazy_mapping", None)
    if lazy_mapping:  # find this workflow's generator job
        for part in lazy_mapping.split(";"):
            info = part.split(":")
            if info[0] == subworkflow_name:
                generator_name = info[1]
                break

    # Update bps_job_summary
    _LOG.debug(
        "Before replace, name = %s, bps_job_summary = %s, add summary = %s, generator_name = %s",
        subworkflow_name,
    ...
    if generator_name:
        ...
        dag_values["bps_job_summary"] = dag_values["bps_job_summary"].replace(
            generator_summary, f"{generator_summary};{subworkflow_summary}"
        )
    else:
        # just append to end of bps_job_summary
        dag_values["bps_job_summary"] += f";{subworkflow_summary}"
