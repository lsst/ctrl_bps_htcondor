...
def _create_periodic_release_expr(
    ...
    * the memory requirements in the last failed run attempt did not reach
    ...
    """
    _LOG.info(
        "MMG: memory: %s, multiplier: %s, limit: %s, additional_expr: %s",
        memory,
        multiplier,
        limit,
        additional_expr,
    ...
    if memory and multiplier and multiplier > 1 and limit:
        ...
        memory_expr = f"{was_mem_exceeded} && {was_below_limit}"
    ...
    if user_expr and memory_expr:
        expr += f" && ({memory_expr} || {user_expr})"
    else:
        expr += f" && {memory_expr}{user_expr}"
...
def _create_periodic_remove_expr(memory, multiplier, limit):
    ...
    * the memory requirements during the last failed run attempt reached
    ...
    mem_expr = ""
    if memory and multiplier and limit:
        mem_expr = f"min({{int({memory} * pow({multiplier}, NumJobStarts - 1)), {limit}}}) == {limit}"
...
def _gather_label_values(config, label):
